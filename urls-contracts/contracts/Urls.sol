...
import {IUrls} from "./interfaces/IUrls.sol";
    ...
    IERC721Receiver
{
    ...
    uint256 public constant PLATFORM_REFERRER_FEE_BPS = 1000; // 10% (of TOTAL_FEE_BPS)
    uint256 public constant ORDER_REFERRER_FEE_BPS = 1000; // 10% (of TOTAL_FEE_BPS)
    uint256 public constant ORIGIN_FEE_BPS = 1000; // 10% (of TOTAL_FEE_BPS)
    ...
    address public immutable originFeeRecipient;
    ...
    constructor(
        ...
        if (_originFeeRecipient == address(0)) revert AddressZero();
    ...
    function initialize(
        ...
        if (_originFeeRecipient == address(0)) {
            _originFeeRecipient = protocolFeeRecipient;
        }
    ...
    ///      - Emit the superset `UrlsTokenTransfer` event with each ERC20 transfer.
    function _update(
        address from,
        address to,
        uint256 value
    ...
    function _disperseFees(uint256 _fee, address _orderReferrer) internal {
        ...
        uint256 originFee = _calculateFee(_fee, ORIGIN_FEE_BPS);
        ...
        bytes4[] memory reasons = new bytes4[](5);

        recipients[0] = tokenCreator;
        amounts[0] = tokenCreatorFee;
        reasons[0] = bytes4(keccak256("URLS_CREATOR_FEE"));

        recipients[1] = platformReferrer;
        amounts[1] = platformReferrerFee;
        reasons[1] = bytes4(keccak256("URLS_PLATFORM_REFERRER_FEE"));

        recipients[2] = _orderReferrer;
        amounts[2] = orderReferrerFee;
        reasons[2] = bytes4(keccak256("URLS_ORDER_REFERRER_FEE"));

        recipients[3] = originFeeRecipient;
        amounts[3] = originFee;
        reasons[3] = bytes4(keccak256("URLS_ORIGIN_FEE"));

        recipients[4] = protocolFeeRecipient;
        amounts[4] = protocolFee;
        reasons[4] = bytes4(keccak256("URLS_PROTOCOL_FEE"));
